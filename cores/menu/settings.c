...
int settings_handle_input(int up, int down, int left, int right, int a, int b, int y) {
    ...
    if (a) {
        // v34: Check for special edit theme ini action - A button also opens editor
        if (settings_selected >= 0 && settings_selected < settings_count) {
            if (strcmp(settings[settings_selected].name, "frogui_edit_theme_ini") == 0) {
                // Open text editor for current GFX theme's theme.ini
                const GfxTheme *theme = gfx_theme_get_current();
...
static void apply_gfx_theme_from_settings(void) {
    // Themes are scanned once at startup, not on every settings load/save
    // This makes theme changes instant instead of taking 4-5 seconds

    // Look for the frogui_gfx_theme setting
    for (int i = 0; i < settings_count; i++) {
        if (strcmp(settings[i].name, "frogui_gfx_theme") == 0) {
            // Find theme by name
            int theme_count = gfx_theme_count();
            for (int t = 0; t < theme_count; t++) {
                if (strcmp(gfx_theme_get_name(t), settings[i].current_value) == 0) {
                    gfx_theme_apply(t);
                    return;
                }
            }
            // Theme not found - use None
            gfx_theme_apply(0);
            return;
        }
    }
    // v72: No setting found - DON'T change current GFX theme
    // This prevents core-specific settings (which don't have frogui_gfx_theme)
    // from disabling the GFX theme when display_opts menu is opened
}
